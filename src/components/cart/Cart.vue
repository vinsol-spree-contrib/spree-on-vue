...
<template slot-scope="scope">
  <el-button type="danger" @click="deleteItem(order.id, scope.row.id)" icon="el-icon-delete" circle>
  </el-button>
</template>
          </el-table-column>
        </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      deleteItem(orderNumber, lineItemId) {
        var _this = this;
        this.$store.dispatch('deleteLineItem', {
          'number': orderNumber,
      ...
      changeCheckoutState(orderNumber, state) {
        ...
        this.$store.dispatch('setCheckoutState', {
          'number': orderNumber,
          'stateData': formData
        });
      }
